...
def _calc_boxplot_stats(distribs, whis=1.5, show_fliers=False):
    ...
    series_stats = []
    for distrib, cum_distrib, cum_distrib_norm in zip(
        distribs, cum_distribs, cum_distribs_norm
    ):
        ...
        if show_fliers:
            ...
            stats["fliers"] = numpy.repeat(
                numpy.arange(len(fliers_indices)), fliers_indices
            )
        series_stats.append(stats)
    return series_stats


def calc_and_plot_boxplot(
    mat,
    by_row=True,
    fhand=None,
    axes=None,
...
def plot_boxplot_from_distribs(
    distribs,
    by_row=True,
    fhand=None,
    axes=None,
...
def plot_boxplot_from_distribs_series(
    distribs_series,
    by_row=True,
    fhand=None,
    axes=None,
    ...
    for dist_n, (distribs, color) in enumerate(zip(distribs_series, colors)):
        ...
        if not by_row:
            mat = mat.transpose()
        positions = numpy.arange(1, (n_series + 1) * mat.shape[0] + 1, n_series + 1)
        positions += dist_n
        bxp_stats = _calc_boxplot_stats(mat)
    ...
    xticks = numpy.arange(
        (n_series + 1) / 2.0, (n_series + 1) * (mat.shape[0] + 1) + 1, n_series + 1
    )
...
def plot_stacked_histograms(
    ...
    **kwargs
    ...
    for idx, (label, this_counts) in enumerate(counts.items()):
        color = cm.Paired(1.0 * idx / len(counts))
        axes.bar(
            edges[:-1],
            this_counts,
            width=width,
...
def plot_barplot(
    ...
    **kwargs
    ...
    axes = df.plot(
        kind="bar", stacked=stacked, figsize=figsize, axes=axes, color=color, **kwargs
    )
    for function_name, params in mpl_params.items():
        function = getattr(axes, function_name)
        function(*params["args"], **params["kwargs"])
...
class _AxesMod:
    ...
    def hist2d(self, matrix, xbins, ybins, log_normed=False):
        ...
        pc = self.axes.pcolormesh(xbins, ybins, matrix, norm=norm, label="hola")
...
def manhattan_plot(
    ...
    if split_by_chrom:
        ...
        return _manhattan_plot_by_chrom(
            chrom,
            pos,
            values,
            mpl_params=mpl_params,
...
def _manhattan_plot(
    ...
    while True:
        ...
        y = values[start:end]
        col = next(colors)
        axes.scatter(x, y, c=col, alpha=0.8, edgecolors="none")
        x_offset = x[-1]
...
def write_curlywhirly(fhand, labels, coords, dim_labels=None, classifications=None):
    sep = "\t"
...
def _estimate_percentiles_from_distrib(
    counts, edges, percentiles=(0.25, 0.50, 0.75), samples_in_rows=True
):
    if counts.ndim == 1:
    ...
    if data_shape == "vector":
        ...
        for idx in range(counts.shape[0]):
            est_quartiles.append(
                numpy.interp(percentiles, run_tot[idx], bins_middle_points)
            )
...
def plot_hists(
    ...
    if counts2 is not None:
        max_per_sample = numpy.maximum(max_per_sample, numpy.max(counts2, axis=0))
    ...
    if plot_quartiles:
        ...
        for cnt_idx, counts in enumerate([counts1, counts2]):
            ...
            percent_vals = _estimate_percentiles_from_distrib(counts, edges, **kwargs)
    ...
    for sample_idx in range(counts1.shape[1]):
        ...
        width = hist_vals
        left = sample_idx + 1
        axes.barh(bottom, width, height, left, zorder=10, alpha=bar_alpha, linewidth=0)
        if counts2 is None:
            axes.barh(
                bottom, -width, height, left, zorder=10, alpha=bar_alpha, linewidth=0
            )
    if counts2 is not None:
...
def plot_sample_missing_het_stats(
    stats, fhand=None, no_interactive_win=False, figsize=None
):
    bar_width = 0.8
    fig, canvas = _get_mplot_fig_and_canvas(fhand, figsize=figsize)
...
def plot_sample_dp_hits(
    stats, fhand=None, no_interactive_win=False, figsize=None, log_axis_for_hists=False
):
    ...
    plot_hists(
        stats["dp_no_missing_counts"],
        stats["bin_edges"],
        axes=axes3,
        counts2=stats["dp_counts"],
